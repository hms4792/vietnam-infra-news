...
send_email.py
=============
Vietnam Infrastructure News - 이메일 알림 발송
Secret 변수명: EMAIL_USERNAME, EMAIL_PASSWORD (기존 등록값 사용)
"""
import smtplib
import os
import sys
...
def get_stats():
    ...
    try:
        ...
        wb.close()
        print(f"[Email] 통계: 전체 {total}건, 금주 {weekly}건")
...
<div style="background:#1a5276;color:white;padding:24px;border-radius:10px 10px 0 0;">
  ...
  <p style="margin:6px 0 0;opacity:0.85;font-size:13px;">
    Daily Pipeline Report &mdash; {now} KST
  </p>
</div>
...
<div style="background:white;padding:24px;border-radius:0 0 10px 10px;">
  ...
  <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
    <tr style="background:#2e86c1;color:white;">
      <th style="padding:10px 14px;text-align:left;">항목</th>
      <th style="padding:10px 14px;text-align:center;">건수</th>
    </tr>
...
def send_email():
    ...
    # ── Secret 변수명: EMAIL_USERNAME, EMAIL_PASSWORD ──────
    # (기존 GitHub Secrets에 등록된 이름 그대로 사용)
    gmail_user = (
    ...
    # 디버그 출력 (값은 노출하지 않음)
    print(f"[Email] EMAIL_USERNAME 있음: {bool(gmail_user)}")
    print(f"[Email] EMAIL_PASSWORD 있음: {bool(gmail_pw)}")

    if not gmail_user or not gmail_pw:
        print("[Email] Secret 미설정 - 이메일 발송 생략")
        sys.exit(0)

    total, weekly = get_stats()

    today_d = datetime.now().strftime("%m/%d")
    subject = f"[베트남 인프라 뉴스] 일일 리포트 - 금주 {weekly}건 수집 ({today_d})"
